...
def check_reachability(
    ...
    """Fast reachability check using link geometry and joint limits only.

    This check is intentionally lightweight (no IK sweeps). It computes a
    conservative radial-distance bound for the bucket tip based on:
      - boom/stick link lengths,
      - stick joint limits,
      - bucket tip offset magnitude.

    It is suitable for visualization pre-filtering where speed matters.
    For exact feasibility (including full coupling of boom/stick/bucket
    limits), use ``check_reachability_exact``.
    """
    target = np.asarray(target_xyz, dtype=float)

    # Transform target into base frame (undo base yaw)
    cos_yaw = math.cos(base_yaw)
    sin_yaw = math.sin(base_yaw)
    dx = target[0] - base_x
    dy = target[1] - base_y
    ...
    # Tip distance from shoulder in swing plane
    r_target = math.sqrt(x_base**2 + y_base**2)
    dr_tip = r_target - SHOULDER_X_LOCAL
    dz_tip = SHOULDER_Z_LOCAL - z_base
    d_tip = math.hypot(dr_tip, dz_tip)

    # Stick-joint-limited wrist distance range
    jdefs = _build_joint_defs()
    stick_def = jdefs['stick_joint']
    low = float(stick_def.lower)
    high = float(stick_def.upper)

    # cos(theta) extrema on [low, high]
    angles = [low, high]
    k_start = int(math.ceil(low / math.pi))
    k_end = int(math.floor(high / math.pi))
    for k in range(k_start, k_end + 1):
        angles.append(k * math.pi)
    cos_vals = [math.cos(a) for a in angles]
    cos_min = min(cos_vals)
    cos_max = max(cos_vals)

    d_wrist_min_sq = L1**2 + L2**2 + 2.0 * L1 * L2 * cos_min
    d_wrist_max_sq = L1**2 + L2**2 + 2.0 * L1 * L2 * cos_max
    d_wrist_min = math.sqrt(max(0.0, d_wrist_min_sq))
    d_wrist_max = math.sqrt(max(0.0, d_wrist_max_sq))

    # Bucket-tip offset magnitude from bucket-joint origin
    tip_offset = math.hypot(BUCKET_LENGTH, BUCKET_DEPTH)

    # Reachable tip-distance interval from shoulder
    if tip_offset < d_wrist_min:
        d_tip_min = d_wrist_min - tip_offset
    elif tip_offset > d_wrist_max:
        d_tip_min = tip_offset - d_wrist_max
    else:
        d_tip_min = 0.0
    d_tip_max = d_wrist_max + tip_offset

    return d_tip_min <= d_tip <= d_tip_max
