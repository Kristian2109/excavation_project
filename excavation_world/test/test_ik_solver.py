...
class TestBasicIK:
    """IK for targets directly in front of the robot (y=0)."""

    def test_straight_ahead_near_ground(self):
...
class TestElbowConfigurations:
    def test_elbow_down_default(self):
        target = np.array([4.0, 0.0, 0.0])
    ...
    def test_nearest_prefers_elbow_down(self):
...
class TestUnreachable:
    ...
    def test_too_close_to_shoulder(self):
...
class TestNonZeroBase:
    ...
    def test_base_at_working_position(self):
...
class TestExcavationTargets:
    """
    Default hole: 4×3×2 m at (5, -2, 0).  Working position: (3, 0, 0).
    Shoulder is near world (4, 0, 1.14).  The arm can reach targets
    roughly 2–5.5 m from the shoulder.  These targets are selected
    to be well within the reachable workspace.
    """
    ...
    def test_excavation_points(self, x, y, z):
...
class TestBucketAngle:
    def test_bucket_angled_45_down(self):
    ...
    def test_bucket_horizontal(self):
    ...
    def test_bucket_angle_sweep_finds_solution(self):
...
class TestVerifyIK:
    ...
    def test_invalid_with_none_joints(self):
...
class TestJointLimits:
    def test_valid_solution_within_limits(self):
        target = np.array([4.0, 0.0, 0.5])
        result = solve_ik_nearest(target)
        if result.success:
            _assert_within_limits(result)

    def test_multiple_targets_within_limits(self):
    ...
    def test_explicit_bucket_angle_limits(self):
